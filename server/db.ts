...
export async function createStripePayment(paymentData: {
  ...
  const result = await db.insert(walletTransactions).values({
    ...
    status: "pending",
    description: `Payment for subscription ${paymentData.subscriptionId}`,
...
export async function updatePaymentStatus(transactionId: number, status: string, stripeStatus?: string) {
  const db = await getDb();
  if (!db) return;
  
  const newStatus = stripeStatus === "succeeded" ? "completed" : stripeStatus === "failed" ? "failed" : status;
...
export async function getPaymentByStripeIntentId(stripePaymentIntentId: string) {
  ...
  const payments = await db.select()
    .from(walletTransactions)
    .where(eq(walletTransactions.description, `Payment for subscription ${stripePaymentIntentId}`))
    .limit(1);
  
  return payments[0] || null;
}

export async function getPaymentsByUser(userId: number, limit: number = 50) {
  const db = await getDb();
  if (!db) return [];
...
export async function getPaymentsBySubscription(subscriptionId: number) {
  ...
  const payments = await db.select()
    .from(walletTransactions)
    .where(eq(walletTransactions.type, "payment"))
    .orderBy(desc(walletTransactions.createdAt));
  
  // Filter by subscriptionId in metadata
